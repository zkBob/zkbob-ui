...
export default () => {
  ...
  useEffect(() => {
    const verifyShieldedAddress = async () => {
      if (zkAccount) {
        let receiverValid = await zkAccount.verifyShieldedAddress(receiver);
        setReceiverValid(receiverValid);
      }
    }
    verifyShieldedAddress().catch(console.error);
  }, [zkAccount, receiver])
